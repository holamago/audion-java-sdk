...
```groovy
implementation 'com.magovoice:audion:0.2.1'
```
...
FlowResponse flow(String flow, String inputType, String input, Integer numSpeakers) throws IOException
```
...
- `numSpeakers` (Integer, 선택): `audion_stt` 화자 수 제한. 생략 또는 `0`은 제한 없음, `1~8`은 지정 화자 수로 처리합니다.
...
FlowResponse flow(String flow, InputStream stream, String filename, long contentLength, Integer numSpeakers) throws IOException
```
...
- `numSpeakers` (Integer, 선택): 화자 수 제한. `0`은 제한 없음, `1~8`은 지정 화자 수
...
# 예제 실행
java -cp "target/audion-0.2.1.jar:target/dependency/*" com.magovoice.audion.Example
```
...
- **v0.2.1**: 화자 수 옵션 지원
  - `audion_stt` flow의 `numSpeakers` 선택 옵션 지원
  - `numSpeakers` 생략 또는 `0` 지정 시 화자 수 제한 없음 처리
- **v0.2.0**: 응답 스키마 확장
